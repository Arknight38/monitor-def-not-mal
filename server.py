"""
PC Monitor Server - Main Entry Point (Fixed)
Refactored modular architecture with proper imports
"""
...
def print_banner():
    ...
    if primary_ip and primary_ip != "0.0.0.0":
        print(f'\nCopy this to your client configuration:')
        print(f'{{')
...
if __name__ == '__main__':
    # Check if running as a service command
    if len(sys.argv) > 1 and sys.argv[1].lower() in ['install', 'remove', 'start', 'stop', 'restart', 'status']:
        print(f"\nService command '{sys.argv[1]}' detected.")
        print("Note: Service installation requires install_service.py module")
        print("Run: python install_service.py " + sys.argv[1])
        print("\nFor now, run the server normally without arguments to start monitoring\n")
        sys.exit(1)
    else:
        main()